...
def load_clip(model_path, device, pretrained=False, context_length=77): 
    """
    FUNCTION: load_clip
    ---------------------------------
    """
...
def zeroshot_classifier(classnames, templates, model, device, context_length=77):
    """
...
def predict(loader, model, zeroshot_weights, device, softmax_eval = True, verbose = 0,
            pred_img_feat = False):
    """
    FUNCTION: predict
    ---------------------------------
    ...
    return y_pred.to("cpu").numpy(), img_path_list


def run_single_prediction(cxr_labels, model, loader, device, template = None, softmax_eval = True,
                          context_length = 77, is_feature_extraction = True):
    """
    ...
    if is_feature_extraction:
        ...
        cxr_phrase = [template]
        zeroshot_weights = zeroshot_classifier(cxr_labels, cxr_phrase, model, device, context_length = context_length)
    
    y_pred, img_path_list = predict(loader, model, zeroshot_weights, device, softmax_eval = softmax_eval, pred_img_feat = is_feature_extraction)
    
    return y_pred, img_path_list
